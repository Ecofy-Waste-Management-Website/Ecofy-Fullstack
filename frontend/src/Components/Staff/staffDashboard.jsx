...
export default function StaffDashboard() {
  ...
  if (!isLoaded || loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-green-50/50">
        <div className="flex flex-col items-center gap-4">
          <div className="h-10 w-10 animate-spin rounded-full border-4 border-green-600 border-t-transparent" />
          <p className="text-green-900 font-bold animate-pulse">Syncing with Schedule...</p>
        </div>
      </div>
    );
  }
  ...
  const renderTaskCard = (task, isCompleted = false) => (
    <div key={task._id} className={`bg-white/90 backdrop-blur-sm rounded-3xl p-6 border border-green-100 shadow-sm transition-all hover:shadow-md ${isCompleted ? 'opacity-80' : ''}`}>
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="text-lg font-bold text-green-950 leading-tight">{task.customer_name || 'Customer'}</h3>
          <p className="text-[10px] font-bold text-green-600/50 uppercase tracking-[0.2em] mt-1">{task.service_type || 'General Service'}</p>
        </div>
        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${isCompleted ? 'bg-green-100 text-green-700' : getStatusColor(task.status)}`}>
          {isCompleted ? 'Completed' : task.status}
        </span>
      </div>

      <div className="bg-green-50/50 rounded-2xl p-4 mb-4 border border-green-100/50">
        <p className="text-[9px] font-bold text-green-600/40 uppercase tracking-widest mb-2 flex items-center gap-1.5"><Icons.MapPin /> LOCATION</p>
        <p className="text-sm text-green-950 font-semibold leading-relaxed truncate">{task.location || 'Location missing'}</p>
      </div>

      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-green-50/30 rounded-2xl p-3 border border-green-100/30">
          <p className="text-[9px] text-green-600/40 font-bold uppercase tracking-widest flex items-center gap-1.5"><Icons.Calendar /> DATE</p>
          <p className="text-xs font-bold text-green-900 mt-1">{isCompleted ? formatDate(task.scheduled_date) : formatDate(task.scheduled_date)}</p>
        </div>
        <div className="bg-green-50/30 rounded-2xl p-3 border border-green-100/30">
          <p className="text-[9px] text-green-600/40 font-bold uppercase tracking-widest flex items-center gap-1.5">
            {isCompleted ? <Icons.Clock /> : <Icons.Clock />} {isCompleted ? 'DONE' : 'TIME'}
          </p>
          <p className="text-xs font-bold text-green-900 mt-1">{isCompleted ? formatTime(task.completedAt) : formatTime(task.scheduled_date)}</p>
        </div>
      </div>
      ...
      {!isCompleted && (
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-3 rounded-2xl text-xs font-bold bg-white border border-green-200 text-green-700 transition-all hover:bg-green-50"
          >
      ...
      <div className="flex h-full w-full gap-3">
        ...
        <aside className="hidden lg:flex flex-col gap-4 bg-green-900/40 backdrop-blur-xl border border-white/10 p-5 text-white w-[240px] shrink-0 rounded-3xl shadow-xl overflow-hidden h-full">
          ...
          <nav className="flex flex-col gap-1.5 overflow-y-auto no-scrollbar flex-1">
            {menuItems.map((item) => (
              ...
              >
                ...
                {item.count > 0 && (
                  <span className="bg-green-900/40 px-2 py-0.5 rounded-full text-[10px] font-bold">
                    {item.count}
                  </span>
                )}
              </button>
          ...
          <div className="mt-auto flex items-center gap-3 rounded-2xl bg-white/10 p-3 text-white border border-white/5 backdrop-blur-sm shrink-0">
            <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-green-700 text-xs font-bold text-white shadow-inner">{staffInitials}</div>
            <div className="min-w-0">
              <p className="m-0 text-[0.65rem] font-bold uppercase tracking-wider text-green-300/80">Staff Portal</p>
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
          ...
          <header className="mb-3 hidden lg:flex flex-row items-center justify-between py-1 px-2 shrink-0">
            ...
            <div className="flex items-center gap-3">
              <div className="relative w-[280px]">
                ...
                <input type="text" className="w-full rounded-2xl border border-green-100 bg-white/60 p-[8px_12px_8px_38px] text-sm outline-none transition-all focus:border-green-500 focus:bg-white focus:shadow-lg focus:shadow-green-900/5 placeholder:text-gray-400" placeholder="Search tasks..." />
              </div>
              <NotificationBell target="staff" />
              <div className="rounded-xl border border-green-100 bg-white/60 px-3 py-1.5 text-xs font-bold text-green-900 backdrop-blur-sm">Staff</div>
              {!roleLoading && role === 'Admin' && (
                <button onClick={handleSwitchDashboard} className="rounded-xl bg-green-600 px-4 py-2 text-xs font-bold text-white transition-all hover:bg-green-700 shadow-md shadow-green-900/10">Switch to Admin</button>
              )}
            </div>
          </header>
          ...
          <main className="flex-1 overflow-y-auto no-scrollbar lg:pr-1">
            <div className="h-full">
              {notification && (
                <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-2xl shadow-xl text-white font-bold text-sm bg-green-600 animate-in fade-in slide-in-from-top-4 duration-300">
              ...
              <section className="grid grid-cols-3 gap-3 mb-4">
                <button 
                  ...
                  className={`flex flex-col items-center p-4 rounded-2xl border transition-all duration-300 bg-white/40 backdrop-blur-md ${
                    activeTab === 'pending' 
                      ? 'border-green-500/60 shadow-[0_0_20px_rgba(16,185,129,0.35)] bg-white/70' 
                      : 'border-green-200/40 hover:bg-white/60 hover:shadow-md'
                  }`}
                >
                  <p className="text-2xl font-bold leading-none text-green-700">
                    {pendingTasks.length}
                  </p>
                  <p className="text-[10px] font-bold uppercase tracking-widest mt-2 text-green-600/60">
                    Pending
                  </p>
                </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {activeTab === 'pending' && (
                  pendingTasks.length === 0 ? (
                    <div className="col-span-full rounded-3xl border border-dashed border-green-200 bg-white/20 p-12 text-center flex flex-col items-center gap-3">
                      <div className="h-12 w-12 rounded-full bg-green-100 flex items-center justify-center text-green-600"><Icons.PendingTasks /></div>
                      <p className="text-green-900/60 font-bold tracking-tight">All caught up! No pending tasks.</p>
                    </div>
                  ) : (
                ...
                {activeTab === 'active' && (
                  ongoingTasks.length === 0 ? (
                    <div className="col-span-full rounded-3xl border border-dashed border-green-200 bg-white/20 p-12 text-center flex flex-col items-center gap-3">
                      <div className="h-12 w-12 rounded-full bg-green-100 flex items-center justify-center text-green-600"><Icons.ActiveTasks /></div>
                      <p className="text-green-900/60 font-bold tracking-tight">No active tasks in progress.</p>
                    </div>
                  ) : (
                ...
                {activeTab === 'completed' && (
                  completedTasks.length === 0 ? (
                    <div className="col-span-full rounded-3xl border border-dashed border-green-200 bg-white/20 p-12 text-center flex flex-col items-center gap-3">
                      <div className="h-12 w-12 rounded-full bg-green-100 flex items-center justify-center text-green-600"><Icons.CompletedTasks /></div>
                      <p className="text-green-900/60 font-bold tracking-tight">No tasks completed yet today.</p>
                    </div>
                  ) : (
              ...
              <footer className="mt-8 text-[0.75rem] text-green-800/40 pb-6 text-center">&copy; 2026 Ecofy Waste Management</footer>
            </div>
          </main>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside className="absolute left-4 top-4 bottom-4 flex w-[86%] max-w-[300px] flex-col gap-4 overflow-y-auto bg-green-900/90 backdrop-blur-xl p-5 text-white shadow-2xl border border-white/10 rounded-3xl">
            ...
            <nav className="flex flex-col gap-1">
              {menuItems.map((item) => (
                <button key={item.key} onClick={() => { setActiveTab(item.key); setIsMobileMenuOpen(false); }} className={`flex justify-between items-center text-left text-sm font-semibold px-4 py-3 rounded-xl transition-all ${activeTab === item.key ? "bg-green-600" : "hover:bg-white/10"}`}>
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-[360px] rounded-3xl bg-white p-6 text-center shadow-2xl">
            <h3 className="mb-2 text-xl font-bold text-green-900 tracking-tight">Confirm Sign Out</h3>
